...
public class StreamingClientStreamingServer {
    static void main() throws Exception {
        ...
        final StreamObserver<Request> request = blockingStubGreeting.streamingClientStreamingServer(new StreamObserver<>() {
            @Override
        ...
        IntStream.range(0, 10).forEach(index -> request.onNext(Request.newBuilder()
                .setRequest(index + ":[" + UUID.randomUUID() + "]").build()));
        request.onCompleted();
